...
/* USER CODE BEGIN PV */
uint32_t adcBuf[1024]={0};
	
uint8_t flag = 0;

float32_t testInput[1024];
static float32_t testOutput[512]={0};
float freq = 0;
uint32_t fftSize = 512;
uint32_t ifftFlag = 0;
uint32_t doBitReserve = 1;
uint32_t testIndex;
float32_t maxValue;
/* USER CODE END PV */
    ...
    /* USER CODE END WHILE */
		...
		if(flag == 1)
		{
			...
			for(int i = 0; i < fftSize; i++)
				{
					testInput[i*2] = (adcBuf[i]&0xfff)*3.3/4095;
					testInput[i*2+1] = 0;
				}
			
			arm_cfft_f32(&arm_cfft_sR_f32_len512, testInput, 0, 1);
			arm_cmplx_mag_f32(testInput,testOutput, fftSize);
			arm_max_f32(&testOutput[1], fftSize-1, &maxValue, &testIndex);	
			freq = 51200/fftSize*(testIndex+1);
