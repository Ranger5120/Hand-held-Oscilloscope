...
void DmTftBase::drawXAxis(uint16_t x0, uint16_t y0, uint16_t color) {
    int x=0, y=0;
    do{
        for(;;){
            x0 = x0 + 6; 
            drawVerticalLine(x0,y0,2,color); 
            if(x0 >= 320) {
                x0 = 0;
                break;
            }
...
void DmTftBase::drawYAxis(uint16_t x0, uint16_t y0, uint16_t color) {
    int x=0, y=0;
    do{
        for(;;){
            y0 = y0 + 6; 
...
void DmTftBase::drawWave(uint16_t x0, uint16_t y0, uint16_t color, double adj) {
    int n = 320, amp = 40;
    int wave[n];
    int x,y;

    for(int i = 0; i < n; i++) {
        wave[i] = std::round(std::sin(adj * 2 * pi * i / n) * amp) ;
    } 
